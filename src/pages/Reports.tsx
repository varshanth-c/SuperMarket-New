...
import { 
  ResponsiveContainer, AreaChart, Area, CartesianGrid, XAxis, YAxis, Tooltip, BarChart,
  Bar, PieChart, Pie, Cell, Sector, LabelList
} from 'recharts';
import { 
    DollarSign, TrendingUp, Package, ShoppingCart, BarChart2, Star, 
    AlertTriangle, Receipt, Banknote, ShieldCheck
} from 'lucide-react';
...
const useAnalyticsData = (salesData: SalesData[], inventoryData: InventoryItem[], expensesData: Expense[]) => {
    // ... (Your existing custom hook code remains unchanged)
...
// 2. Define animation variants for the staggered effect
const staggerContainerVariants = {
    hidden: { opacity: 0 },
    visible: {
        opacity: 1,
        transition: {
            staggerChildren: 0.1, // This will delay each child's animation by 0.1s
        },
    },
};
...
const Reports = () => {
  ...
  // ... (Your existing useQuery hooks remain unchanged)
  ...
  return (
    <div className="flex min-h-screen w-full flex-col bg-slate-50 dark:bg-slate-950">
      <Navbar />
      {/* 3. Apply the main page transition */}
      <motion.main 
      ...
      >
        ...
        {/* 4. Apply the staggered animation to the card container */}
        <motion.div 
            className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-5"
            variants={staggerContainerVariants}
            initial="hidden"
            animate="visible"
        >
            <motion.div variants={staggerItemVariants}><Card className="border-l-4 border-green-500 hover:shadow-lg transition-shadow"><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Total Revenue</CardTitle><DollarSign className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold">{formatCurrency(totalRevenue)}</div></CardContent></Card></motion.div>
            ...
            <motion.div variants={staggerItemVariants}><Card className="border-l-4 border-red-500 hover:shadow-lg transition-shadow"><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Expenses</CardTitle><Banknote className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold">{formatCurrency(totalExpenses)}</div></CardContent></Card></motion.div>
            <motion.div variants={staggerItemVariants}><Card className="border-l-4 border-blue-500 hover:shadow-lg transition-shadow"><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Net Profit</CardTitle><TrendingUp className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className={`text-2xl font-bold ${netProfit >= 0 ? 'text-blue-600' : 'text-red-600'}`}>{formatCurrency(netProfit)}</div></CardContent></Card></motion.div>
        ...
        <div className="mx-auto grid w-full max-w-7xl auto-rows-fr grid-cols-1 gap-6 lg:grid-cols-5">
            {/* ... Your charts and other cards go here ... */}
